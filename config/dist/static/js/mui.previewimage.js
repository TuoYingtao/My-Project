(function($, window) {
	...
	var template = '<div id="{{id}}" class="muiMin-slider muiMin-preview-image muiMin-fullscreen"><div class="muiMin-preview-header">{{header}}</div><div class="muiMin-slider-group"></div><div class="muiMin-preview-footer muiMin-hidden">{{footer}}</div><div class="muiMin-preview-loading"><span class="muiMin-spinner muiMin-spinner-white"></span></div></div>';
	var itemTemplate = '<div class="muiMin-slider-item muiMin-zoom-wrapper {{className}}"><div class="muiMin-zoom-scroller"><img src="{{src}}" data-preview-lazyload="{{lazyload}}" style="{{style}}" class="muiMin-zoom"></div></div>';
	...
	var PreviewImage = function(options) {
		this.options = $.extend(true, {
			...
			zoom: true,
			header: '<span class="muiMin-preview-indicator"></span>',
			footer: ''
		}, options || {});
		this.init();
		this.initEvent();
	...
	proto.initEvent = function() {
		...
		this.element.addEventListener('slide', function(e) {
			if (self.options.zoom) {
				var lastZoomerEl = self.element.querySelector('.muiMin-zoom-wrapper:nth-child(' + (self.lastIndex + 1) + ')');
				if (lastZoomerEl) {
